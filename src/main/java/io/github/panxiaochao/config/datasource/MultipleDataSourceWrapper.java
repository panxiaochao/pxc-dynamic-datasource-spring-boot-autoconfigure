package io.github.panxiaochao.config.datasource;

import com.alibaba.druid.pool.DruidDataSource;
import io.github.panxiaochao.enums.DbSourceEnum;
import io.github.panxiaochao.exception.MultipleDataSourceException;
import io.github.panxiaochao.multiple.DbSourceContextHolder;
import io.github.panxiaochao.multiple.DynamicDataSource;
...
public class MultipleDataSourceWrapper {
    private static final Logger LOGGER = LoggerFactory.getLogger(MultipleDataSourceWrapper.class);
    ...
    public DataSource build() {
        LOGGER.info(">>> build");
        // 设置多数据源target
        Map<String, DataSource> dataSourceMap = bindProperties();
     ...
     */
    private Map<String, DataSource> bindProperties() {
        LOGGER.info(">>> bindProperties");
        ...
        if (DbSourceContextHolder.DATA_SOURCE_LIST.size() > DbSourceEnum.values().length) {
            throw new MultipleDataSourceException("默认支持1个master，5个db数据库！");
        }
